...
이 프린터는 [SN-04 인덕티브 센서](http://nasspop.co.kr/products/4595513263)를 프로브로 사용한다. 간트리 레벨링과 베드 레벨링에 이 센서를 사용한다. 자세한 사항은 배선도를 참조한다.

이 프린터는 2기의 100와트 [PTC 히터](http://59051988.com/products/5372976301)를 사용해 챔버를 가열한다. *PTC의 특성상 과열로 인한 손상이 불가능하기 때문에 이는 베드 히터에 병렬로 결선되어 별도의 제어가 필요하지 않다.*

이 프린터는 후면의 필터를 통해 내부 공기를 여과해 배출한다. 필터에 붙은 팬이 멈추면 내부의 fume이 여과되지 않는다. ~~필터 유닛은 시중의 적당한 [HEPA 필터](https://smartstore.naver.com/qualityno1/products/4833831971)를 구입해 절단하여 만들어야 한다.~~

이 프린터는 PEI 코팅된 스프링 [철판](https://smartstore.naver.com/3d-plus/products/4684859943)을 출력면으로 사용한다. PEI는 물리적인 손상에 극히 약하므로 주의해서 관리해야 한다.
...
시타델은 [SKR v1.3 보드](https://smartstore.naver.com/3d-plus/products/4735883538)를 마이크로컨트롤러로 사용하고 [라즈베리파이 4B](https://smartstore.naver.com/icfactory/products/4750528335)를 서버로 사용한다.
...
*시타델은 시설 내 `DHL`이라는 SSID를 가진 라우터에 연결되어있다.*

-	Wi-Fi 연결 설정을 바꾸고 싶을 시 리눅스 터미널에서 `raspi-config`을 사용한다.
